module wesad_dwn_top #(parameter NUM_MODALITY = 5, parameter CLK_FREQ = 12_000_000)(
    input logic clk,
    input logic [NUM_MODALITY - 1:0] dr,
    input logic miso,
    input logic rst,
    output logic sclk,
    output logic mosi, // irrelevant
    output logic [NUM_MODALITY - 1:0] cs,
...
spi_SR shift_register (
    .clk,
    .rst,
    .data_in(miso),
    .sample_en,
    .data_out(sensor_write_data)
);

logic [15:0] sensor_write_addr;
logic wren_spram;
logic [15:0] read_data; // not used yet
...
logic armed, hop_en, ingest_idle, ingest_hold;
...
// cs is all ones only while the ingest FSM sits in IDLE
assign ingest_idle = &cs;

hop_timer #(.HOP(1), .CLK_HZ(CLK_FREQ)) hop (
    .clk,
    .rst(rst | ~armed),
    .hop_en
);
